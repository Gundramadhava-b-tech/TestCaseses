...
async function seedFirebase() {
  console.log(`🔥 Seeding Firebase Firestore project [${projectId}]...`);
  ...
  for (let i = 0; i < patients.length; i++) {
    const p = patients[i];
    const patientRef = db.collection("patients").doc(p.id);
    ...
    await patientRef.set(patientDocData, { merge: true });
    console.log(`  ✓ Patient added to Firebase Firestore: ${p.name} (ID: ${p.id})`);
    ...
    for (const scan of scanSet) {
      ...
      const scanDocData = {
        ...
        status: "completed",
        uploadedAt: admin.firestore.Timestamp.fromDate(uploadedAtDate),
      };

      await db.collection("scans").doc(scanId).set(scanDocData, { merge: true });
      ...
      const analysisDocData = {
        ...
        analyzedAt: admin.firestore.Timestamp.fromDate(analyzedAtDate),
      };

      await db.collection("analyses").doc(analysisId).set(analysisDocData, { merge: true });
    }
  }
